...
def initialize_net_and_optims(target_device, cfg):
    ...
    for i_mo_sol in range(0, n_mo_sol):
        net_list.append(mo_regression.Net(target_device))
...
def forward_propagation(net_list, net_optimizer_list, loss_fn):
    """
    ...
    for i_mo_sol in range(len(net_list)):
        ...
        Y_hat = net_list[i_mo_sol].forward()
        loss_per_sample = loss_fn(Y_hat)  #obj, n_samples
...
def run(mo_optimizer_name, target_device, cfg):
    ...
    for i_mo_sol in range(len(net_list)):
        Y_hat = net_list[i_mo_sol].forward()
        loss = loss_fn(Y_hat) #n_mo_obj * n_samples
